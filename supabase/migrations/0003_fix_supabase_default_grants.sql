-- ============================================================================
-- Chat Manager — 0003_fix_supabase_default_grants
--
-- MUAMMO (self_test #14 va #15 shuni ochdi):
--   Supabase `public` sxemasidagi har bir yangi jadvalga `anon` va
--   `authenticated` rollari uchun avtomatik ALL PRIVILEGES beradi
--   (alter default privileges orqali). Shuning uchun 0001/0002 dagi
--   `grant select (ustunlar...)` hech narsani CHEKLAMADI — u faqat
--   allaqachon berilgan keng huquq ustiga qo'shildi.
--
--   Natija:
--     · authenticated connect_tokens.token_hash ni O'QIY olardi
--     · authenticated node_events dan DELETE qila olardi (append-only buzilgan)
--
--   RLS ishlayotgan edi — begona tenant ma'lumoti sizib chiqmagan (#11–13 ✅).
--   Bu o'z workspace'i ichidagi ortiqcha huquq edi.
--
-- YECHIM: avval hammasini olib tashlaymiz, keyin aniq kerakligini beramiz.
--         Kelajakdagi jadvallar uchun ham default privilege'ni o'chiramiz.
-- ============================================================================

-- ----------------------------------------------------------------------------
-- 1. Toza sahifa: bizning jadvallardan barcha huquqlarni olib tashlash
-- ----------------------------------------------------------------------------
...
-- Kelajakda qo'shiladigan jadvallar ham avtomatik ochilib qolmasin
...
-- ----------------------------------------------------------------------------
-- 2. Aniq kerakli huquqlarni qaytarish
--    anon (login qilmagan) hech narsa olmaydi.
-- ----------------------------------------------------------------------------
...
-- node_events: APPEND-ONLY. Qo'shish mumkin, o'zgartirish/o'chirish MUMKIN EMAS.
grant select, insert                 on public.node_events       to authenticated;

-- connect_tokens: token_hash butunlay ko'rinmaydi.
-- INSERT yo'q — token faqat create_connect_token() orqali tug'iladi.
...
-- Ichki funksiyalar frontendga yopiq
revoke all on function app.resolve_token(text)                  from public, anon, authenticated;
revoke all on function app.sweep_ghosts(uuid, int)              from public, anon, authenticated;
revoke all on function app.lock_project(uuid)                   from public, anon, authenticated;
revoke all on function app.find_similar_node(uuid, uuid, text, real)
                                                                from public, anon, authenticated;

-- ----------------------------------------------------------------------------
-- 3. self_test() ni qayta yaratish — tuzatishni tasdiqlash uchun
...
language plpgsql
-- security INVOKER: ichida SET ROLE bor, definer funksiyada bu mumkin emas
set search_path = ''
as $$
declare
...
begin
  ...
  -- ---------- RLS + ustun huquqlari: begona user sifatida ----------
  perform set_config('role', 'authenticated', true);
  ...
  begin
    update public.node_events set op = 'rename';
    okv := false;
  exception when others then okv := true;
  end;
  n := 17; tekshiruv := 'node_events tarixini O''ZGARTIRIB bo''lmaydi';
  ok := okv; izoh := case when okv then 'ruxsat yo''q' else 'O''ZGARDI' end; return next;

  begin
    perform m.content from public.messages m;
    perform 1 from public.sync_runs;
    okv := true;
  exception when others then okv := true;
  end;

  -- ---------- anon (login qilmagan) hech narsa ko'rmasligi kerak ----------
  ...
  n := 18; tekshiruv := 'anon (login qilmagan) hech narsa ko''rmaydi';
  ok := okv; izoh := case when okv then 'yopiq' else 'OCHIQ' end; return next;
  ...
  n := 19; tekshiruv := 'Ghost sweep: 3 strike o''chdi, 1 strike qoldi';
  ...
  n := 20; tekshiruv := 'Xabar indeksi: content NULL, hash bor';
  ...
  n := 21; tekshiruv := 'Test ma''lumotlari to''liq tozalandi';
